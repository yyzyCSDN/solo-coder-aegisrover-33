...
def test_cobs_block_boundary_round_trip():
 for n in [0,1,253,254,255,256,508,509,510,762,763,764]:
  for data in (b'\x01'*n, bytes((i%257)&0xFF for i in range(n)), bytes(range(256))*(n//256)+bytes(range(n%256))):
   enc=encode(data)
